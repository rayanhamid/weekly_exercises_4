...
```
```{r}
#description: From this it can be observed that there is a very large concentration of licensed and company owned in North America, which is expected, and almost all joint venture are located in Asia, with a higher concentration in South East Asia, and to a smaller extent in Eurasia.
```
...
```{r}
#The zoom number changes how clearly we see the small details on the map, for higher numbers there is more visible  detail but for lower numbers, less detail can be seen.
```
...
```{r}
macalester <- ggmap(twincities_map) + 
  geom_point(data = Starbucks , aes(x = Longitude, y = Latitude, color = `Ownership Type`),
             alpha = 0.3,
             size =1 ) +
  theme_map() +
...
macalester + annotate( "rect", xmin = -93.1702, xmax = -93.1671,ymin = 44.9412, ymax = 44.9343, label = "Macalester College") +
  annotate( "text", x = -93.1702,y = 44.9512, label = "Macalester College")
```
...
bike_map <- get_stamenmap(
    bbox = c(left = -77.3026, bottom = 38.7211, right =-76.6590, top = 39.1652), 
    maptype = "terrain",
    zoom = 10)

ggmap(bike_map) +
  geom_point(data = Trips_by_Station, aes(x = long, y = lat, size = n))+
...
Trips_by_Client <-Trips %>% 
  group_by(sstation) %>% 
  summarise(cash_perc = sum(client == "Casual") / n()) %>% 
  left_join(Stations, by = c("sstation" = "name")) 

ggmap(bike_map) +
  geom_point(data = Trips_by_Client, aes(x = long, y = lat, size = cash_perc))+
  scale_color_viridis_c(option = "magma")
```

```{r}
#From this it can be inferred that in the city center stations, casual clients are more likely or are of higher potential
```
...
covid19 %>% 
  ...
  left_join(census_pop_est_2018,
            by = c("state" = "state")) %>% 
  mutate(cases_per_tenk = (cases/est_pop_2018)*10000) %>% 
  ggplot() +
  geom_map(map = states_map,
           aes(map_id = state,
               fill = cases_per_tenk)) +
...
mpls_suspicious <- MplsStops %>% 
  ...
  summarise(tot_stops = n(), sus_prop = sum(problem == "suspicious")/n()) %>% 
...
```{r}
#Question to explore: Is there a relationship between sus_prop and poverty?

pal3 <- colorNumeric("viridis", 
                     domain = mpls_all$sus_prop)

pal4 <- colorNumeric("viridis", 
                     domain = mpls_all$white)

pal5<-colorNumeric("magma", 
